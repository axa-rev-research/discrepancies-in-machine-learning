...
cdef inline void _init_split(SplitRecord* self, SIZE_t start_pos, SIZE_t start_pos_expo) nogil:
    self.impurity_left = INFINITY
    self.impurity_right = INFINITY
    self.pos = start_pos
    self.pos_expo = start_pos_expo
    self.feature = 0
    self.threshold = 0.
...
cdef class Splitter:
    ...
    def __cinit__(self, Criterion criterion, SIZE_t max_features,
        ...
        self.samples = NULL
        self.samples_expo = NULL #HERE
        self.n_samples = 0
        self.n_samples_expo = 0 # HERE
        self.features = NULL
        self.n_features = 0
        self.feature_values = NULL
        self.feature_values_expo = NULL #HERE

        self.sample_weight = NULL

        self.max_features = max_features
        self.min_samples_leaf = min_samples_leaf
        self.min_samples_expo = min_samples_expo #HERE
    ...
    def __dealloc__(self):
        ...
        free(self.samples)
        free(self.samples_expo) #HERE
        free(self.features)
        free(self.constant_features)
        free(self.feature_values)
        free(self.feature_values_expo) #HERE
    ...
    cdef int init(self,
                   object X,
                   const DOUBLE_t[:, ::1] y,
                   DOUBLE_t* sample_weight, object X_expo=None) except -1: #HERE
        ...
        cdef SIZE_t n_samples_expo = X_expo.shape[0] #HERE
        
        
        # Create a new array which will be used to store nonzero
        # samples from the feature of interest
        cdef SIZE_t* samples = safe_realloc(&self.samples, n_samples) #HERE LOOK? self.samples? existe déjà
        cdef SIZE_t* samples_expo = safe_realloc(&self.samples_expo, n_samples_expo) #HERE ????

        cdef SIZE_t i, j
        cdef double weighted_n_samples = 0.0
        cdef double weighted_n_samples_expo = 0.0 #HERE
        j = 0
        ...
        #HERE
        j = 0
        for i in range(n_samples_expo):
            # Only work with positively weighted samples
            samples_expo[j] = i
            j += 1

            weighted_n_samples_expo += 1.0
        self.n_samples_expo = n_samples_expo
        
        
        self.weighted_n_samples = weighted_n_samples
        self.weighted_n_samples_expo = weighted_n_samples_expo #HERE
        ...
        safe_realloc(&self.feature_values_expo, n_samples_expo) #HERE
    ...
    cdef int node_reset(self, SIZE_t start, SIZE_t end,# SIZE_t start_expo, SIZE_t end_expo, #HERE
        ...
        self.start = start
        self.end = end
        #self.start_expo = start_expo
        #self.end_expo = end_expo
    ...
    cdef int node_split(self, double impurity, SplitRecord* split,
                        SIZE_t* n_constant_features, SIZE_t start_expo, SIZE_t end_expo #HERE
                       ) nogil except -1:
...
cdef class BaseDenseSplitter(Splitter):
    ...
    cdef const DTYPE_t[:, :] X_expo #HERE

    cdef SIZE_t n_total_samples
    cdef SIZE_t n_total_samples_expo #HERE
    ...
    cdef int init(self,
        ...
        # Call parent init
        Splitter.init(self, X, y, sample_weight, X_expo) #HERE

        self.X = X
        self.X_expo = X_expo #HERE
        return 0
...
cdef class BestSplitter(BaseDenseSplitter):
    ...
    def __reduce__(self):
        return (BestSplitter, (self.criterion,
                               ...
                               self.min_samples_expo, #HERE
                               self.min_weight_leaf,
                               self.random_state), self.__getstate__())

    cdef int node_split(self, double impurity, SplitRecord* split,
                        SIZE_t* n_constant_features, SIZE_t start_expo, SIZE_t end_expo,#HERE TEST workaround
                       ) nogil except -1:
        """Find the best split on node samples[start:end]
        ...
        cdef SIZE_t* samples_expo = self.samples_expo #HERE
        cdef SIZE_t start = self.start
        cdef SIZE_t end = self.end
        #cdef SIZE_t start_expo = self.start_expo #HERE
        #cdef SIZE_t end_expo = self.end_expo #HERE
        ...
        cdef DTYPE_t* Xf_expo = self.feature_values_expo #HERE
        ...
        cdef SIZE_t f_j
        cdef SIZE_t p
        cdef SIZE_t p_expo #HERE
        cdef SIZE_t current_pos_expo #HERE
        ...
        cdef SIZE_t partition_end_expo #HERE

        _init_split(&best, end, end_expo)
        ...
        while (f_i > n_total_constants and  # Stop early if remaining features
            ...
            if f_j < n_known_constants:
                ...
                #HERE                    
                for i in range(start_expo, end_expo):
                    Xf_expo[i] = self.X_expo[samples_expo[i], current.feature]
                    
                  
                sort(Xf + start, samples + start, end - start)
                sort(Xf_expo + start_expo, samples_expo + start_expo, end_expo - start_expo) #HERE
                ...
                if Xf[end - 1] <= Xf[start] + FEATURE_THRESHOLD:
                    ...
                    while p < end:
                        ...
                        p += 1
                        #HERE
                        #Faut il increemnter aussi p_expo?
                        ...
                        if p < end:
                            ...
                            p_expo = start_expo #HERE
                            while ((Xf_expo[p_expo] <= (Xf[p - 1] / 2.0 + Xf[p] / 2.0)) and (p_expo + 1 < end_expo)):
                                p_expo += 1
                            p_expo -= 1
                            
                            current.pos_expo = p_expo
                                
                            if (((current.pos_expo - start_expo) < min_samples_expo) or
                                    ((end_expo - current.pos_expo) < min_samples_expo)):
                                continue
        ...
        #HERE
        if best.pos_expo < end_expo:
            partition_end_expo = end_expo
            p_expo = start_expo
            ...
            #HERE
            while p_expo < partition_end_expo:
                if self.X_expo[samples_expo[p_expo], best.feature] <= best.threshold:
                    p_expo += 1

                else:
                    partition_end_expo -= 1

                    samples_expo[p_expo], samples_expo[partition_end_expo] = samples_expo[partition_end_expo], samples_expo[p_expo]
...
cdef class RandomSplitter(BaseDenseSplitter):
    ...
    cdef int node_split(self, double impurity, SplitRecord* split,
        ...
        cdef SIZE_t start = self.start
        cdef SIZE_t end = self.end
        #cdef SIZE_t start_expo = self.start_expo
        #cdef SIZE_t end_expo = self.end_expo
