...
cdef class TreeBuilder:
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
                np.ndarray sample_weight=None, object X_expo=None):
        """Build a decision tree from the training set (X, y)."""
        pass

    cdef inline _check_input(self, object X, np.ndarray y,
                             np.ndarray sample_weight, object X_expo=None):
        ...
        if (sample_weight is not None and
            ...
            not sample_weight.flags.contiguous)):
                sample_weight = np.asarray(sample_weight, dtype=DOUBLE,
                                           order="C")
        if X_expo is not None:
            if X_expo.dtype != DTYPE:
                # since we have to copy we will make it fortran for efficiency
                X_expo = np.asfortranarray(X_expo, dtype=DTYPE)      
            
        return X, y, sample_weight, X_expo
...
cdef class DepthFirstTreeBuilder(TreeBuilder):
    ...
    def __cinit__(self, Splitter splitter, SIZE_t min_samples_split,
                  SIZE_t min_samples_leaf, SIZE_t min_samples_expo, double min_weight_leaf,
                  SIZE_t max_depth, double min_impurity_decrease):
        self.splitter = splitter
        self.min_samples_split = min_samples_split
        self.min_samples_leaf = min_samples_leaf
        #HERE AND ABOVE
        self.min_samples_expo = min_samples_expo
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
        ...
        # check input
        #HERE
        X, y, sample_weight, X_expo = self._check_input(X, y, sample_weight, X_expo)
        ...
        #HERE
        cdef SIZE_t min_samples_expo = self.min_samples_expo
        ...
        #HERE
        splitter.init(X, y, sample_weight_ptr, X_expo)


        cdef SIZE_t start
        cdef SIZE_t end
        cdef SIZE_t start_expo #HERE
        cdef SIZE_t end_expo #HERE
        cdef SIZE_t depth
        cdef SIZE_t parent
        cdef bint is_left
        cdef SIZE_t n_node_samples = splitter.n_samples
        #HERE 
        # "appliquer" le splitter au X_expo, et prendre la taille correspondante
        cdef SIZE_t n_node_samples_expo = splitter.n_samples_expo
        ...
        with nogil:
            ...
            rc = stack.push(0, n_node_samples, 0, n_node_samples_expo, #HERE last2
                            0, _TREE_UNDEFINED, 0, INFINITY, 0) 
            if rc == -1:
            ...
            while not stack.is_empty():
                ...
                start_expo = stack_record.start_expo #HERE
                end_expo = stack_record.end_expo #HERE
                ...
                splitter.node_reset(start, end, #start_expo, end_expo,
                                    &weighted_n_node_samples) #HERE ! should be uncommented! on essaie sans cette classe.
                
                #HERE TODO!!!
                n_node_samples_expo = end_expo - start_expo#HERETODO
                
                
                is_leaf = (depth >= max_depth or
                           n_node_samples < min_samples_split or
                           n_node_samples < 2 * min_samples_leaf or
                           n_node_samples_expo < 2 * min_samples_expo or
                ...
                if not is_leaf:
                    splitter.node_split(impurity, &split, &n_constant_features, start_expo, end_expo)#HERE,added 2
                ...
                node_id = tree._add_node(parent, is_left, is_leaf, split.feature,
                                         split.threshold, impurity, n_node_samples, n_node_samples_expo, #HERE
                ...
                if not is_leaf:
                    ...
                    rc = stack.push(split.pos, end, split.pos_expo, end_expo, #HERE
                                    depth + 1, node_id, 0,
                    ...
                    rc = stack.push(start, split.pos, start_expo, split.pos_expo, #HERE
                                    depth + 1, node_id, 1,
...
cdef inline int _add_to_frontier(PriorityHeapRecord* rec,
    ...
    or 0 otherwise.
    """
    return frontier.push(rec.node_id, rec.start, rec.end, rec.start_expo, rec.end_expo, #HERE
                         rec.pos, rec.pos_expo, #HERE
                         rec.depth,
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    def __cinit__(self, Splitter splitter, SIZE_t min_samples_split,
                  SIZE_t min_samples_leaf, SIZE_t min_samples_expo, #HERE
                  min_weight_leaf,
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
                np.ndarray sample_weight=None, object X_expo=None): #HERE
        """Build a decision tree from the training set (X, y)."""

        # check input
        X, y, sample_weight, X_expo = self._check_input(X, y, sample_weight, X_expo) #HERE
        ...
        cdef SIZE_t min_samples_leaf = self.min_samples_leaf
        cdef SIZE_t min_samples_expo = self.min_samples_expo #HERE
        cdef double min_weight_leaf = self.min_weight_leaf
        cdef SIZE_t min_samples_split = self.min_samples_split
        print("ai je qqc ici???AVANT INIT", splitter.start, splitter.end, splitter.start_expo, splitter.end_expo, "oui, toujours en C je crois?")
        # Recursive partition (without actual recursion)
        splitter.init(X, y, sample_weight_ptr, X_expo) #HERE
        print("ai je qqc ici???", splitter.start, splitter.end, splitter.start_expo, splitter.end_expo, "oui, idem")
        ...
        cdef SIZE_t n_node_samples = splitter.n_samples
        cdef SIZE_t n_node_samples_expo = splitter.n_samples_expo #HERE
        ...
        with nogil:
            ...
            rc = self._add_split_node(splitter, tree, 0, n_node_samples, 0, n_node_samples_expo, #HERE
            ...
            while not frontier.is_empty():
                ...
                if is_leaf:
                    ...
                    rc = self._add_split_node(splitter, tree,
                                              record.start, record.pos, record.start_expo, record.pos_expo, #HERE
                    ...
                    rc = self._add_split_node(splitter, tree, record.pos,
                                              record.end, record.pos_expo, record.end_expo, #HERE
    ...
    cdef inline int _add_split_node(self, Splitter splitter, Tree tree,
                                    SIZE_t start, SIZE_t end, SIZE_t start_expo, SIZE_t end_expo, 
                                    double impurity,
        ...
        cdef SIZE_t n_node_samples_expo #HERE
        ...
        n_node_samples = end - start
        #HERE???
        n_node_samples_expo = end_expo - start_expo
        
        # HERE 2e check de is_leaf, quand on ajoute un split. On check alors qu'on peut splitter en 2
        is_leaf = (depth >= self.max_depth or
                   n_node_samples < self.min_samples_split or
                   n_node_samples < 2 * self.min_samples_leaf or
                   n_node_samples_expo < 2 * self.min_samples_expo or #HERE
                   weighted_n_node_samples < 2 * self.min_weight_leaf or
                   impurity <= EPSILON  # impurity == 0 with tolerance
                   )

        if not is_leaf:
            splitter.node_split(impurity, &split, &n_constant_features, start_expo, end_expo)#HERE
        ...
        node_id = tree._add_node(parent - tree.nodes
                                 ...
                                 is_left, is_leaf,
                                 split.feature, split.threshold, impurity, n_node_samples, n_node_samples_expo, #HERE
                                 weighted_n_node_samples)
        if node_id == SIZE_MAX:
            return -1
        ...
        res.start = start
        res.end = end
        res.start_expo = start_expo #HERE
        res.end_expo = end_expo #HERE
        res.depth = depth
        ...
        if not is_leaf:
            # is split node
            res.pos = split.pos
            res.pos_expo = split.pos_expo #HERE
            res.is_leaf = 0
...
cdef class Tree:
    ...
    #HERE
    property n_node_samples_expo:
        def __get__(self):
            return self._get_node_ndarray()['n_node_samples_expo'][:self.node_count_expo]
    ...
    cdef SIZE_t _add_node(self, SIZE_t parent, bint is_left, bint is_leaf,
                          ...
                          SIZE_t n_node_samples_expo, #HERE
        ...
        #HERE
        node.n_node_samples_expo = n_node_samples_expo
    ...
    with nogil:
        ...
        while not stack.is_empty():
            ...
            if child_l[node_idx] == _TREE_LEAF:
                ...
                rc = stack.push(child_l[node_idx], 0, 0, 0, 0, node_idx, 0, 0, 0)# HERE AU PIF
                if rc == -1:
                    with gil:
                        raise MemoryError("pruning tree")

                rc = stack.push(child_r[node_idx], 0, 0, 0, 0, node_idx, 0, 0, 0) #HERE AU PIF
                if rc == -1:
                    with gil:
                        raise MemoryError("pruning tree")
        ...
        while candidate_nodes[0]:
            ...
            rc = stack.push(pruned_branch_node_idx, 0, 0, 0, 0, 0, 0, 0, 0) #HERE AU PIF
            if rc == -1:
                with gil:
                    raise MemoryError("pruning tree")
            ...
            while not stack.is_empty():
                ...
                if child_l[node_idx] != _TREE_LEAF:
                    # ... and child_r[node_idx] != _TREE_LEAF:
                    rc = stack.push(child_l[node_idx], 0, 0, 0, 0, 0, 0, 0, 0) #HERE AU PIF
                    if rc == -1:
                        with gil:
                            raise MemoryError("pruning tree")
                    rc = stack.push(child_r[node_idx], 0, 0, 0, 0, 0, 0, 0, 0) #HERE AU PIF
                    if rc == -1:
                        with gil:
                            raise MemoryError("pruning tree")
...
cdef _build_pruned_tree(
    ...
    with nogil:
        # push root node onto stack
        rc = stack.push(0, 0, 0, 0, 0, _TREE_UNDEFINED, 0, 0.0, 0) #HERE au pif
        if rc == -1:
            with gil:
                raise MemoryError("pruning tree")
        ...
        while not stack.is_empty():
            ...
            new_node_id = tree._add_node(
                ...
                node.impurity, node.n_node_samples, node.n_node_samples_expo, #HERE
            ...
            if not is_leaf:
                # Push right child on stack
                rc = stack.push(
                    node.right_child, 0, 0, 0, depth + 1, new_node_id, 0, 0.0, 0)#HERE AU PIF
                if rc == -1:
                    break

                # push left child on stack
                rc = stack.push(
                    node.left_child, 0, 0, 0, depth + 1, new_node_id, 1, 0.0, 0)#HERE AU PIF
                if rc == -1:
                    break
