...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(
        self, X, y, sample_weight=None, check_input=True, X_idx_sorted="deprecated", X_expo=None
    ):
        ...
        #HERE... Ugly but convenient at first
        if X_expo is None:
            X_expo = X.copy()
            self.min_samples_expo = self.min_samples_leaf
        else:
            X_expo = np.array(X_expo)

        # Determine output settings
        n_samples, self.n_features_in_ = X.shape
        #HERE
        n_samples_expo = X_expo.shape[0]
        ...
        #HERE
        if isinstance(self.min_samples_expo, numbers.Integral):
            if not 0 <= self.min_samples_expo:
                raise ValueError(
                    "min_samples_expo must be at least 1 or in [0, 0.5], got %s"
                    % self.min_samples_expo
                )
            min_samples_expo = self.min_samples_expo
        else:  # float
            if not 0.0 <= self.min_samples_expo <= 0.5:
                raise ValueError(
                    "min_samples_expo must be at least 1 or in [0, 0.5], got %s"
                    % self.min_samples_expo
                )
            min_samples_expo = int(ceil(self.min_samples_expo * n_samples_expo))
        ...
        min_samples_split = max(min_samples_split, 2 * min_samples_leaf, 2 * min_samples_expo)
        ...
        #HERE
        builder.build(self.tree_, X, y, sample_weight, X_expo)
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            #HERE
            min_samples_expo=min_samples_expo,
