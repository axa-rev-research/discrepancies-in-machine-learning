...
class pool2graph:
    ...
    def __init__(self, Xtrain, Ytrain, pool, discrepancy_regression_threshold, different_regression_threshold, k_init=10):
        ...
        self.discrepancy_regression_threshold = discrepancy_regression_threshold
        self.different_regression_threshold = different_regression_threshold
    ...
    def fit(self, max_epochs=0):
        ...
        _preds = self.pool.predict(self.Xtrain, mode='regression') #HERE
        _preds.index = self.Xtrain.index
        _discrepancies = self.pool.predict_discrepancies(self.Xtrain, threshold=self.discrepancy_regression_threshold)
    ...
    def get_edges_kneighbors(self, k=None):
        ...
        lnodes_DP = {i:{'discrepancies':n['discrepancies'], 'pred':n['pool_predictions'].mean()} for i,n in nodes} #HERE
        lnodes_DP = pd.DataFrame(lnodes_DP).T
        if k is None:
            k = self.k_init

        # (1) Process points with discrepancies: their nearest neighbors can be any points, no matter the discrepancies or the predicted labels # NOCHANGE HERE
        ...
        pred_means = lnodes_DP[lnodes_DP.discrepancies == 0].pred #HERE Basically we treat mean preds from non-disc obs as classes
        
        for p in pred_means.unique(): #HERE    
            X_nn = nodes_features[~((lnodes_DP.pred - p).abs() > self.different_regression_threshold) | (lnodes_DP.discrepancies==1)] #HERE
    ...
    def _refine_graph(self):
        ...
        w_preds = self.pool.predict(w, mode='regression')
    ...
    def _edge_selection(self, e, policy='discrepancy+differentPredictions'):
        ...
        if policy == 'regression':
            selected = ((abs(self.G.nodes[e[0]]['pool_predictions'].mean() - self.G.nodes[e[1]]['pool_predictions'].mean()) > self.different_regression_threshold) or (self.G.nodes[e[0]]['discrepancies']==1 or self.G.nodes[e[1]]['discrepancies']==1))
