...
'''
Idea for categorical variables: create 2 middles (copies of parents) instead of 1.
L'idée ici est de lancer avant l'itération 0 la création des autres arêtes, en préprocessing.
...
class pool2graph:
    ...
    def fit(self, max_epochs=0):
        ...
        if len(self.categorical_names) == 0:
            ...
            _edges = self.get_edges_kneighbors_categorical()        
    ...
    def get_edges_kneighbors_categorical(self, k=None):
        ...
        ### ici on a déjà qui est relié avec qui, donc on peut bosser ici
        # pour chaque edge
            # vérifier si catégorielles différentes; si non, on laisse le edge originel
            # vérifier si en changeant les catégorielles, on garde les mêmes classes prédites et discrepancies aussi
            # si oui à ces conditions
            # on crée les copies des noeuds
            # on crée des edges entre les nouveaux et anciens --> ca peut etre pas?
            # on supprime les anciens edges
            # on ajoute les nouveaux noeuds
            # on ajoute les nouveaux edges
        _edges_new = []
        for _e in _edges:
            u_idx, v_idx = _e[0], _e[1]
            
            if nodes_features.loc[u_idx][self.categorical_names].all() == nodes_features.loc[v_idx][self.categorical_names].all():
                _edges_new = _edges_new + [_e]
                print('equal test')
                import pdb;pdb.set_trace()
            else:
                print('not equal')
                u2_features = nodes_features[u_idx].copy()
                v2_features = nodes_features[v_idx].copy()
                u2_features[self.categorical_names] = v2_features[self.categorical_names].copy()
                v2_features[self.categorical_names] = u2_features[self.categorical_names].copy()
                
                if (self.pool.predict(u2_features.reshape(-1, 1), mode='classification') == self.pool.predict(nodes_features[u_idx].reshape(-1, 1), mode='classification')) and (self.pool.predict_discrepancies(u2_features.reshape(-1, 1)) == self.pool.predict_discrepancies(nodes_features[u_idx]).reshape(-1, 1)):
                    self.new_nodes_index += -1
                    _new_node = [(self.new_nodes_index,
                        {"features":u2_features,
                        "pool_predictions":self.pool.predict(u2_features.values.reshape(-1, 1), mode='classification'),
                        "discrepancies":self.pool.predict_discrepancies(u2_features.values.reshape(-1, 1)),
                        "y_true":None,
                        "ground_truth":False,
                        "Xtrain_index":self.new_nodes_index})]
                    self.G.add_nodes_from(_new_node)
                    
                    _new_edge = [(self.new_nodes_index, v_idx, {'distance': 1})]
                    _edges_new += _new_edge
                
                if (self.pool.predict(v2_features.reshape(-1, 1), mode='classification') == self.pool.predict(nodes_features[v_idx].reshape(-1, 1), mode='classification')) and (self.pool.predict_discrepancies(v2_features.reshape(-1, 1)) == self.pool.predict_discrepancies(nodes_features[v_idx]).reshape(-1, 1)):
                    self.new_nodes_index += -1
                    _new_node = [(self.new_nodes_index,
                        {"features":v2_features,
                        "pool_predictions":self.pool.predict(v2_features.values.reshape(-1, 1), mode='classification'),
                        "discrepancies":self.pool.predict_discrepancies(v2_features.values.reshape(-1, 1)),
                        "y_true":None,
                        "ground_truth":False,
                        "Xtrain_index":self.new_nodes_index})]
                    self.G.add_nodes_from(_new_node)
                    
                    _new_edge = [(self.new_nodes_index, u_idx, {'distance': 1})]
                    _edges_new += _new_edge                
    ...
    def _refine_graph(self):
        ...
        # créer 2 milieux en copiant les catégorielles des parents. Le milieu n'est fait que sur les continues
        #u, v, w = [],[],[]
        u, v, w1, w2 = [],[],[],[] 
        w = []
        for i in range(len(to_refine)):
            ...
            #w.append((u[-1]+v[-1])/2.)
            w_cont = (u[-1][self.continuous_names] + v[-1][self.continuous_names])/2.
            w1.append(w_cont.append(u[-1][self.categorical_names]))
            w2.append(w_cont.append(v[-1][self.categorical_names]))
        ...
        w1_preds = self.pool.predict(w1, mode='classification')
        w2_preds = self.pool.predict(w2, mode='classification')
        w1_discrepancies = self.pool.predict_discrepancies(w1)
        w2_discrepancies = self.pool.predict_discrepancies(w2)

        d_uw1 = np.linalg.norm(u-w1, axis=1)
        d_vw1 = np.linalg.norm(v-w1, axis=1)
        d_uw2 = np.linalg.norm(u-w2, axis=1)
        d_vw2 = np.linalg.norm(v-w2, axis=1)

        w1 = pd.DataFrame(np.array(w1), columns=self.Xtrain.columns)
        w2 = pd.DataFrame(np.array(w2), columns=self.Xtrain.columns)
        ...
        for i in range(len(w1)):
            ...
            self.new_nodes_index += -1
            features1 = w1.iloc[i]
            features1.name = self.new_nodes_index #watch out here cause order of features might be different?...
            
            new_node = (self.new_nodes_index,
            {'pool_predictions':w1_preds.iloc[i],
            'features':features1,
            'discrepancies':w1_discrepancies.iloc[i],
            'y_true':None,
            'ground_truth':False,
            'Xtrain_index':self.new_nodes_index})
            ...
            self.new_nodes_index += -1
            features2 = w2.iloc[i]
            features2.name = self.new_nodes_index #watch out here cause order of features might be different?...
            
            new_node = (self.new_nodes_index,
            {'pool_predictions':w2_preds.iloc[i],
            'features':features2,
            'discrepancies':w2_discrepancies.iloc[i],
            'y_true':None,
            'ground_truth':False,
            'Xtrain_index':self.new_nodes_index})
        ...
        new_edges11 = [(to_refine[i,1][0], new_nodes[2*i,0], {'distance':d_uw1[i]}) for i in range(to_refine.shape[0])] # on crée qu'une seule liste new_nodes pour compter les index?
        new_edges12 = [(to_refine[i,1][1], new_nodes[2*i,0], {'distance':d_vw1[i]}) for i in range(to_refine.shape[0])]
        
        new_edges21 = [(to_refine[i,1][0], new_nodes[2*i+1,0], {'distance':d_uw2[i]}) for i in range(to_refine.shape[0])]
        new_edges22 = [(to_refine[i,1][1], new_nodes[2*i+1,0], {'distance':d_vw2[i]}) for i in range(to_refine.shape[0])]

        new_edges = new_edges11 + new_edges12 + new_edges21 + new_edges22
