...
'''
Idea for categorical: select 1 categorical value of parent and keep the same.
Issue: criterion for edges to refine does not really make sens, cause edges are not split in the middle anymore.
On a essayé ici de faire le vrai milieu, c'est à dire de compenser un éventuel changement de catégorielle sur les continues, mais en fait c'est con ça ne marche pas (problème sans solution dans certains cas)
'''
class pool2graph:
    ...
    def fit(self, max_epochs=0):
        ...
        ### ICI on a identifié les aretes initiales. Donc en fait on voudrait identifier les noeuds, créer les copies aux catégorielles changées pour les arêtes aux catégorielles différents, obtenir pour chaque arête deux nouvelles arêtes avec catégorielle fixée. Puis lancer la suite.
        ###
    ...
    def _refine_graph(self):
        ...
        # rien ne devrait avoir changé ici
        u, v, w = [],[],[]
        for i in range(len(to_refine)):
            u.append(self.G.nodes[to_refine[i,1][0]]['features'])
            v.append(self.G.nodes[to_refine[i,1][1]]['features'])
            #w.append((u[-1]+v[-1])/2.)
            w_cat = [u, v][np.random.randint(2)][-1][self.categorical_names] # meme proba entre les deux parents
            # to do, pas la meme proba exactement
            
            which_parent = w_cat.all()==v[-1][self.categorical_names].all() # = 1 si w descend de v, 0 si descend de u
            print(which_parent)
            cat_pen = np.linalg.norm(w_cat-[u,v][1 - which_parent][-1][self.categorical_names], axis=None)**2 #on calcule la penalisation = distance categorielle entre w et le parent dont il ne descend pas
            
            w_cont = (cat_pen/len(self.continuous_names) + np.power([u,v][1 - which_parent][-1][self.continuous_names], 2) - np.power([u,v][which_parent][-1][self.continuous_names], 2)) / (2 * ([u,v][1 - which_parent][-1][self.continuous_names] - [u,v][which_parent][-1][self.continuous_names]))
            # formule pour split non pas au milieu des continues mais au vrai milieu si changement dans les categorielles
